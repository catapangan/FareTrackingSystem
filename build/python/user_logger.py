...
class RFIDLogger(QThread):
	...
	def __init__(self, parent=None):
		...
		self.invalid_statusText		= "You have insufficient balance"
	...
	def get_local_accounts(self, conn, cursor, userid):
		try:
			cursor.execute("SELECT firstname, lastname, usertype, tag, balance FROM user_accounts WHERE userid='%s' LIMIT 1" % (userid))
		...
		return (True, accounts_data[0]['balance'], accounts_data[0]['firstname'], accounts_data[0]['lastname'], accounts_data[0]['usertype'], accounts_data[0]['tag'])
	...
	def compute_fare(self, conn, cursor, balance, user_data, loc_data, usertype):
		try:
			...
			if total_distance <= 4.0:
				...
				elif usertype == 'special':
					fare = self.fare_base_discount
			else:
				if usertype == 'regular':
					fare = self.fare_base_regular + (total_distance - 4.0) * self.fare_rate_regular
				elif usertype == 'special':
					fare = self.fare_base_discount + (total_distance - 4.0) * self.fare_rate_discount
	...
	def process_userid(self, user_array):
		...
		try:
			...
			(isSuccess, balance, firstname, lastname, usertype, tag) = self.get_local_accounts(conn, cursor, userid)
			if isSuccess == False:
				raise Exception("Accounts error")
				
			if tag == -2:
				self.updateMessage.emit(self.invalid_statusText, "")
			else:
				if balance < 0 and tag == -1:
					cursor.execute("UPDATE user_accounts SET tag=-2 WHERE userid=%s" % (user_data['userid']))
					conn.commit()
				...
				for key, value in settings_data:
					if key == 'max_fare':
						self.max_fare = value
					elif key == 'reg_base_fare':
						self.fare_base_regular  = value
					elif key == 'spc_base_fare':
						self.fare_base_discount  = value
					elif key == 'reg_rate_perkm':
						self.fare_rate_regular  = value
					elif key == 'spc_rate_perkm':
						self.fare_rate_discount  = value
				
				if user_data[0]['status'] == 0 and datetime.datetime.now() - datetime.timedelta(hours=4) > userdata[0]['date']:
					balance = balance - self.max_fare
					user_data[0]['status'] = 1
				...
				if len(loc_data) > 0:
					if len(user_data) > 0:
						if user_data[0]['status'] == 0:
							...
							(isSuccess, fare, distance, points, isValid) = self.compute_fare(conn, cursor, balance, user_data[0], loc_data[0], usertype)
							if isSuccess == False:
								raise Exception("Fare computation error")
							if isValid:
								self.current_statusText = self.compute_successText + firstname + ' ' + lastname
								self.current_resultText = self.compute_resultText + str('%.2f' % fare) + ' for ' + str('%.2f' % distance) + 'km'
							else:
								self.current_statusText = self.compute_failureText + firstname + ' ' + lastname
								self.current_resultText = self.compute_resultText + str('%.2f' % fare) + ' for ' + str('%.2f' % distance) + 'km'
								cursor.execute("UPDATE user_accounts SET tag=-1 WHERE userid=%s" % (user_data['userid']))
								conn.commit()
							...
							self.current_statusText = self.welcome_statusText + firstname + ' ' + lastname
